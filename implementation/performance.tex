...
\begin{itemize}
   ...
   \item Twitter~\cite{snapnets}, a graph with 81306 nodes and 1768149 edges.
      The shortest distance is calculuated for 40 nodes; 

   \item EU Email~\cite{snapnets}, a graph with 265000 nodes and 420000
      edges. The shortest distance is calculated for 100 nodes;

   \item Pokec~\cite{snapnets}, a large graph representing data from a popular
      social network website in Slovakia. The graph has 1632803 nodes and 30622564
      edges.
...
than the C++ version and usually slower than LM.
...
score and corresponding move. The LM version is seven times slower due to the
costs of creating new nodes using the exists construct and the large memory
requirements as shown next. In Chapter~\ref{chapter:coordination} we will show
how to improve the space complexity of the MiniMax program and the corresponding
run time. An interesting aspect of the MiniMax program is that it does not use
any persistent predicates and therefore does not use array data structures for
managing facts, which have show to improve the run time of several programs in
our experiments.
...
facts with unique lists around. Notably, the C++ memory usage is not much
smaller than LM's.
...
root) since the \textbf{Final} memory usage is much smaller than the
...
per fact, we see that the MSSD requires on average 500B, in which the hash table
data structures used for indexing are a big part of. For the US Power Grid
dataset, where 24 million facts are derived, the average memory per fact drops
to just 230B, which is just seven times more than the 32B required to store a
