...
is 15KB, which also includes those freed nodes. Note that the memory usage of
...
data structures used for indexing are a big part of. For the Orkut dataset,
where 100 million facts are derived, the average memory per fact is exactly 30B,
which is about the 32B required to store a particular linear fact for
representing one shortest distance.
