...
\texttt{@1} is not active, then thread 2 also needs to activate \code{@1} by
...
\begin{algorithm}[H]
   ...
   $N.db\_lock.lock()$\;
   $N.main\_lock()$\;
   \tcc{Add facts from the temporary Fact Buffer into the database}
