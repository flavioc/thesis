...
computed using $T_1/T_i$, where $i$ represents the number of threads used and
...
with a 60-fold speedup for 32 threads. This is because of the default ordering
used for 1 thread which results in a slower convergence rate, resulting in
super-linear speedups. When using multiple threads, the graph will converge
faster because belief updates have more up-to-date information from neighbor
nodes due to the existence of multiple threads.  Note that we also used the same
computation ordering in the C++ program, which is also the one used in the
GraphLab framework.
...
absolute performance of the LM program that was noted in the previous section.
...
configuration has less scalability than the 120x120 dataset due to the smaller
size. For the 120x120 dataset, LM has a 14-fold speedup for 32 threads, which
...
networks with 81306 nodes and 1768149 edges and which was already used before in
the MSSD program.  Since Twitter is a much smaller dataset, its speedup is much
smaller than Google Plus, as expected. However, while the speedup of Twitter
starts to drop after 16 threads, the LM system is still able to make it faster
by adding more threads.  For Google Plus, a 22-fold speedup is achieved for 32
threads, which is reasonable scalability considering that GGC is not a program
where work is equally distributed.
...
row. For a more in-depth explaination of the N-Queens program please see
...
lists which are shared between threads. This introduces cache line misses
because the reference count of each list data structure needs to be updated when
new solutions are constructed. The 14-Queens configuration has the best run time
when using 16 to 20 threads, while the 13-Queens configuration performs best
with 14 threads. One can argue that the best number of threads is correlated to
the size of the board, because the second half of execution is better balanced
is better partitioned when the number of threads matches the size of the board.
...
experiments has only 32GB, the Small configuration scales due to the smaller
memory requirements. In Section~\ref{section:coord:minimax}, we will show how
this program can be improved by changing the default scheduling order of LM and
improving the memory usage of the program.
...
speedup for 32, the best from the 4 datasets used. The second conclusion is that
...
than C++ when using at least 5 threads, which is fairly impressive result. We
think this is because, in this particular graph, the number of edges is almost
...
\begin{figure}[h]
        ...
        when there is more work to do, with the US Power Grid dataset requiring
     more execution time and the best scalability.}
...
more scalable with a 18-fold speedup for 32 threads.
