...
pointers (\code{previous} and \code{next}) and a variable number of arguments.
...
\item \emph{Trie Data Structures} are used to store persistent facts. Tries are
   trees where facts are indexed by common prefix arguments. The \code{previous}
...
\item \emph{Array Data Structures} are used to store persistent facts that are
   ...
   data structure do not have the \code{previous} and \code{next} pointers
...
\item \emph{Doubly Linked List Data Structures} are used to store linear facts.
   ...
   remove facts. The \code{previous} and \code{next} pointers are used to chain
