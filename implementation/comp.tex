...
iterator is simply the fact that can be iterated by following the \code{next}
...
To illustrate how rules are compiled, consider again the rule of the
...
matched until implicit constraint is true. First, the \code{colored} linked list
is iterated over by first retrieving the head fact and then using the
\code{next} pointer of each fact. Inside this outer loop, a nested \emph{while}
...
new \code{colored} fact is derived and two used linear facts are consumed by
erasing them from the linked lists. Note that after the rule is derived, the
code must \code{return} to terminate the rule since there is a higher priority
rule that may be triggered with the new \code{colored} fact (see
Fig.~\ref{language:code:bichecking}). This enforces the priority semantics of
the language.
    
If the constraint had failed, another \code{visit} fact would have been used by
assigning \code{visit} to \code{visit.next()}.  Likewise, if the initial
...
filters out the \code{visit} facts with the same color.  Note that now have a
\emph{goto statement} (line~\ref{line:implementation:goto}) that is executed
when the rule is derived. In this case, since no new \code{colored} fact was
