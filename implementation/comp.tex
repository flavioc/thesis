...
\code{goto next} to jump to the outer loop which uses the next \code{colored}
fact computed using \code{colored\_list.erase(colored)}. This forces the
...
being compiled cannot be the same since the derived \code{colored} fact
is used in the LHS of a higher priority rule, namely, the rule in 
line~\ref{line:implementation:higher}. Once the \code{colored} fact is derived,
...
filters out the \code{visit} facts with the same color.
