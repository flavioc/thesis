...
The compilation algorithm iterates through the fact expressions in rule's LHS
and creates nested loops to try all the possible combinations of facts.  For
this rule, all the pairs of facts \texttt{shortest} and \texttt{relax} must be
...
Fig.~\ref{code:shortest_path_program}). This enforces the priority semantics of
the language.
...
above, the fact \texttt{shortest(A, D1, P1)} is re-derived in the rule's RHS.
Our compiler is able to turn linear loops into persistent loops for linear facts
...
(line~\ref{line:implementation:goto}) that is executed when the rule is derived.
...
with $n$ comprehensions or aggregates, where $CB_i$ and $CH_i$ is the LHS and
RHS of the comprehension/aggregate, respectively, and $H$ represents the atomic
propositions found in rule's RHS. The formula used by the compiler to detect
conflicts between predicates is the following:
...
for(auto it1(linked_list("pagerank").begin()); it1 != linked_list("pagerank").end(); )
{
   ...
   for(auto it2(linked_list("update").begin()); it2 != linked_list("update").end(); )
   {
      ...
      for(auto it3(linked_list("neighbor-pagerank").begin()); it3 !=
         ...
         fact *f3(*it3);
         acc += f3->get_float(2);
         ++it3; // the sub-rule has no RHS since neighbor-pagerank is re-derived
      }
...
they appear in the LHS or RHS of the rule.  Coordination facts in the LHS are
compiled into code that inspects the state of the virtual machine. For example,
the fact \texttt{priority} will inspect the target node, retrieve the current
priority and assign the priority to a variable.  Coordination facts in the RHS
are also implemented as API commands of the virtual machine instead of being
added to the database as facts. Semantically, action facts are like any other.
However, since they are immediately used by the machine, there is no need to
store them in the database, therefore avoiding unnecessary allocations and
deallocations.
