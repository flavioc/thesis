In order to support priorities and node partitioning, the work queue of each
...
For the coordination facts \code{set-priority} and \code{add-priority}, we
...
As an example, if a node derives two \code{set-priority} facts that change the
