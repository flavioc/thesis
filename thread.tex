In the last chapter, we have introduced new language facilities that can be used
by the programmer to coordinate execution. This new approach retains the
implicit parallelism of the standard LM language but it still does not allow the
programmer to fully reason about the underlying parallel architecture.
Coordination facts such as \code{thread-id} and \code{set-thread} both use the
type \code{thread} in its argument, which refers to a thread in the system.
