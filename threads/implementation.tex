To support thread-based facts, the compilation and runtime system described in
...
implicit \code{running(T, N)} fact, the fact that a different node is being
executed, is enough to fire mixed rules. Without using an implicit
\code{running} fact, the system would need to lookup for a regular node that
