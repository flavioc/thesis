...
and predicates. The \code{consumer} and \code{generator} types become a subtype
...
the system and make each thread assume the ownership of its generators. Each
...
Figure~\ref{fig:threads:powergrid} shows how the example PowerGrid 
...
show the run time of the version without thread facts, named \textbf{Regular}, and the
...
The second important observation relates the last two datasets where we
