...
First, we define the initial axioms: \code{owner(A, T)} on
...
\code{F = true} if node \code{A} has axioms, otherwise \code{F = false};
...
thread \code{T} is running on node \code{A}.
...
\code{schedule-next} coordination fact, which simply re-derives a
\code{set-priority} but with an infinite priority. Fact \code{set-priority} is
processed in lines~\ref{line:threads:model_set1}-\ref{line:threads:model_set2}
by updating the priority values in the \code{priority} facts. As explained in
