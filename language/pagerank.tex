...
declaring six predicates, namely: \code{outbound}, to declare outbound links
where to third argument is a weight that depends on the number of outbounds
links for the source page; \code{numInbound}, with the number of inbound links;
\code{pagerank}, to represent the PageRank of a page; \code{accumulator}, to
accumulate the incoming neighbor's PageRank values; \code{neighbor-pagerank}, to
represent an incoming PageRank value; and \code{start} to initialize a node.
...
links of a page. The constant \code{iterations} reads the number of iterations
to execute from the program's input arguments and \code{pages} is assigned to
\code{@world}, a special constant that evaluates to the number of nodes in the
...
nodes.  The \code{\bang outbound(A, B, W)} fact forms a $n \times n$ square
matrix (number of nodes) and is the so-called \scare{Google Matrix}.  All the
