...
nodes as uncolored; \code{colored}, to mark as colores; \code{fail}, to mark an
invalid bipartite graph; and \code{visit} to perform the coloring of the graph.
Initially, all nodes in the graph start as \code{uncolored}, because they do not
have a color yet. The axiom \code{visit(@1, 1)} is instantiated at node
\code{@1} (line~\ref{line:language:bc_axiom}) in order to color it with color 1.
