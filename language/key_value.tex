...
predicate \code{value} stores the key/value pair of a node. The linear predicate
\code{replace} represents an update operation where the key in the second
argument is to be updated to the value in the third argument. Predicates
\code{left} and \code{right} represent the child nodes of each BST node.
...
the right branch. The derivation of \code{replace} facts on node \code{B} can
also be seen as a implicit \emph{message passing} from \code{A} to \code{B},
since \code{B} is a different node than \code{A} and the LHS of each rule can
...
tree configuration, including keys and values, and the \code{replace(@0, 6,
"x")} fact instantiated at the root node \code{@3} that manifests the intent to
...
\begin{figure}[h]
        ...
        \begin{subfigure}[b]{0.5\textwidth}
                ...
                \caption{After applying rule 3 at node \code{@0}. The
                \code{replace} fact is \emph{sent} to (or derived at) node
                \code{@2}.}
        ...
        \begin{subfigure}[b]{0.5\textwidth}
                ...
                \caption{After applying rule 3 at node \code{@2}. Replace fact
                   reaches node \code{@6}.}
        ...
        \begin{subfigure}[b]{0.5\textwidth}
                  ...
                  \caption{After applying rule 1 at node \code{@6}. Value of key
                  6 has changed to "x".}
...
Like the key/value example shown in the previous section, the amount of
