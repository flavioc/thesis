...
From a fact \code{test-y}$(A, Y, State, OriginalState)$, there are two possible
scenarios:
...
\begin{lemma}[test-diag-left lemma]
   From a fact \code{test-diag-left}$(A, X, Y, State, OriginalState)$, there are
  ...
  \begin{itemize}
     ...
     \item The \code{test-diag-left} is retracted and is used to derive a
        \code{test-diag-right}$(A, X_0 - 1, Y_0 + 1, OriginalState, OriginalState)$
...
From a fact \code{test-diag-right}$(A, X, Y, State, OriginalState)$, there are
two possible scenarios:
...
\begin{itemize}
   ...
   \item The \code{test-diag-right} fact is retracted and the fact
      \code{send-down}$(A, [(X_0, Y_0) | OriginalState])$ is derived.
\end{itemize}
\end{lemma}
