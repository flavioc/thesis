...
Belief values are arrays of floats and are represented by \code{belief/2}
...
belief values of the neighbor nodes, we use \code{neighbor-belief/3} facts,
...
\code{update/1} fact starts the process. The first rule (lines 27) simply
consumes redundant \code{update/1} facts and the second rule (lines 30-37)
...
\code{neighbor-belief-old} are fully consumed, the rule in line 24 is fired in
order to consume \code{update-messages}.

For each neighbor update, we also check in lines 5-8 if the change in belief
values is greater than \code{bound} (a program constant) and then force the
neighbor nodes to update their belief values by deriving \code{update(B)}.
...
\code{add-priority} in order to increase to priority of neighbor nodes when
...
language expression \code{1} (lines 33, 41 and 49), which corresponds to an
empty rule head. If there are no conflicts, this means that it is possible to
add a queen to the current state (line 31). The fact \code{send-down/2} is
