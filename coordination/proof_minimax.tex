...
\code{new-score}$(A, OtherScore, OtherPlay)$, the program's rule will retract
...
Case 2 ($N > 0$): by picking an arbitrary \code{new-score}$(A, OtherScore,
OtherPlay)$ fact.
...
\code{new-score}$(A, OtherScore, OtherPlay)$ facts, the program's rules will
...
\code{play} fact of each child $B$ to get a $\mathtt{score}(B, Score, Play)$
...
We also derived $\mathtt{maximize}(A, N, mininf, 0)$ fact (or \code{minimize})
