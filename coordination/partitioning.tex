...
\begin{itemize}
   ...
   \item \code{thread-id(node A, thread T)}: Store at node \texttt{A} which
   ...
   \item \code{just-moved(node A)}: Linear fact derived after the
      \code{set-thread} action is performed and the node is running on the
      target thread.
