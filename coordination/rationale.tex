In motivate the need for coordination, we present the Single
...
1-3) with the declaration of the predicates. The first predicate, \code{edge},
...
program computes the shortest distance from node \code{@1} to all other nodes
in the graph. Every node has a \code{shortest} fact that is improved with new
\code{relax} facts.  Lines 5-9 declare the axioms of the program:
\code{edge} facts describe the graph; \code{shortest(A, +00, [])} is the
initial shortest distance (infinity) for all nodes; and \code{relax(@1, 0,
   [@1])} starts the algorithm by setting the distance from \code{@1} to
\code{@1} to be 0.
...
\code{shortest} path \code{P1} with distance \code{D1} is larger than a
new path \code{relax} with distance \code{D2}, then replace the current
shortest path with \code{D2}, delete the new \code{relax} path and propagate
new paths to the neighbors (lines 13-14).  The comprehension iterates over the
edges of node \code{A} and derives a new \code{relax} fact for each node
\code{B} with the distance \code{D2 + W}, where \code{W} is the weight of
...
\code{@2} and \code{@3}. Fig.~\ref{fig:shortest_path_program}~(b) is the
result after applying the same rule but at node \code{2}.

The second rule of the program (lines 16-17) retracts a \code{relax} fact
...
is possible to either apply rules in either node \code{@2} or node
\code{@3}. This decision depends largely on implementation factors such as
