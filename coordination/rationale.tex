To motivate the need for coordination, we present the Single Source Shortest
Path~(SSSP), a concise program that can take advantage of custom scheduling
policies to improve its performance. Note that in
Section~\ref{section:implementation:performance}, we have already mentioned the
MSSD program which is essentially the SSSP modified to compute multiple shortest
distances.

The SSSP program starts
...
initial facts of the program: \code{edge} facts describe the graph;
\code{shortest(A, +00, [])} is the initial shortest distance (infinity) for all
nodes; and \code{relax(@1, 0, [@1])} starts the algorithm by setting the
distance from \code{@1} to \code{@1} to be 0.
