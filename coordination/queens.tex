...
squares of the board should have a several \code{final-state} facts which
...
(line~\ref{line:coord:queens_add}). The fact \code{send-down} is used to
...
Fig.~\ref{coordination:fig:nqueens}. The full proof of correctness for the
N-Queens program is presented in Appendix~\ref{appendix:proofs:nqueens}.
...
\caption{Propagation of states using the \code{send-down} fact.}
