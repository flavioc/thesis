...
on classical logics, such as the \emph{predicate and propositional calculus}, or
...
\paragraph{Parallel Functional Programming}
...
transactional memory}~\cite{harris2005composable} have shown to be more
...
fine-grain granularity problem. Usually, these coarse-grained parallel
...
communication using mechanisms such as software transactional
...
therefore $Q$ must also be true. For example, if we know that ``it is raining'' and that ``if
...
\paragraph{Parallel Logic Programming}
...
and $R = \mathtt{prop}_2(\hat{x}, \hat{y})$ then the set variables $\hat{x}$ must be
the same in the two propositions. This issue does not arise in OR-parallelism,
however AND-parallelism may be better when the program is more deterministic
(i.e., it contains less alternative implications).
...
languages.
MapReduce~\cite{Dean:2008:MSD:1327452.1327492}, for instance, is a
